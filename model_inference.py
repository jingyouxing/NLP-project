...
parser.add_argument('--gpus', type=str, default=None, help='GPU IDs to use (e.g., 0 or 0,1). If None, use CPU.')
...
def get_response_from_all():
    ...
    def generate_response_per_model(model_name):
        ...
        if isinstance(languages,str):
            ...
            generate_response(model_name,model_path,tokenizer,model,languages,countries,questions,question_col,id_col,output_dir,prompt_no=None)
        else:
            for l,c in zip(languages,countries):
                questions = get_questions(l,c)
                generate_response(model_name,model_path,tokenizer,model,l,c,questions,question_col,id_col,output_dir,prompt_no=None)
